...
module.exports = function logoranSwagger(config) {
  ...
  const logoranSwaggerUi = function(ctx, next) {
    if ((options.routePrefix === false && ctx.path === '/') || ctx.path === options.routePrefix) {
      ctx.type = 'text/html';
  ...
  if (!extFavicon16) {
    logoranSwaggerUi.path.push(defaultOptions.favicon16);
  }
  if (!extFavicon32) {
    logoranSwaggerUi.path.push(defaultOptions.favicon32);
  }

  return logoranSwaggerUi;
};
