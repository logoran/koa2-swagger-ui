...
module.exports = function logoranSwagger(config) {
  ...
  if (!extFavicon16 && options.routePrefix) {
    options.favicon16 = path.join(options.routePrefix, defaultOptions.favicon16);
  }
  if (!extFavicon32 && options.routePrefix) {
    options.favicon32 = path.join(options.routePrefix, defaultOptions.favicon32);
  }
  ...
  return function logoranSwaggerUi(ctx, next) {
    if (options.routePrefix === false || ctx.path === options.routePrefix || ctx.path === '/') {
      ctx.type = 'text/html';
      ctx.body = index(options);
      return true;
    }
    if (!extFavicon16 && (ctx.path === defaultOptions.favicon16 || ctx.path === options.favicon16)) {
      ctx.type = 'image/png';
      ctx.body = fs.createReadStream(favicon16Path);
      return true;
    }
    if (!extFavicon32 && (ctx.path === defaultOptions.favicon32 || ctx.path === options.favicon32)) {
      ctx.type = 'image/png';
